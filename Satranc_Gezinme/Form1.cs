...
namespace Satranc_Gezinme
{
    public partial class Form1 : Form
    {
        ...
        private void Tiklama(object sender, EventArgs e)
        {
            ...
            if (snc != 1)
            {
                ...
                for (int i = 0; i < 8; i++)
                {
                    if (renk[i] != "bos")
                    {
                        kontrol = renk[i].Split(',');
                        int sr = Convert.ToInt32(kontrol[0]); int sn = Convert.ToInt32(kontrol[1]);
                        // dizi[sr, sn] = 0;
                        foreach (var item in pnlEkran.Controls)
                        {
                            if (item is Button)
        ...
        private void btnCiz_Click(object sender, EventArgs e)
        {
            ...
            else if (rdbAlti.Checked == true)
            {
                satir = 6; sutun = 6;
                dizi = new int[satir, sutun];
            }
            else if (rdbBes.Checked == true)
            {
                satir = 5; sutun = 5;
                dizi = new int[satir, sutun];
            }
